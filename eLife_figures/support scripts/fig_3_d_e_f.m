...
colormap(flip(hot))
h1 = colorbar
ylabel(h1,'toxic fraction','Rotation',90)
%%
monitorPos = get(0,'MonitorPositions'); % [x y w ht] X # of monitors
w = 900; ht = 200; % Define window size: width(w) and height(ht)
mon = 1; % Only one monitor
...
ylabel('GC activation (%)'),xlabel('vessel diameter (\mum)'),title('proximal')
...
ylabel('GC activation (%)'),xlabel('vessel diameter (\mum)'),title('regional')
axis square, caxis([0 1]),axis([10 50 5 95])
...
ylabel('GC activation (%)'),xlabel('vessel diameter (\mum)'),title('uniform')
axis square, caxis([0 1]),axis([10 50 5 95])
yticklabels({'10','20','30','40','50','60','70','80','90'})
yticks([10:10:90])
hold on
colormap(flip(fliplr(hot)))
h2 = colorbar
ylabel(h2,'toxic fraction','Rotation',90)
end
