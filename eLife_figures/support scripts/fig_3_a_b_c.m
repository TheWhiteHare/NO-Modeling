...
fig = figure('name','Figure 3 a,b,c Perivascular CcO inhibition','NumberTitle','off',...
    'position',pos,'color','w');
set(fig,'defaultAxesColorOrder',[0 0 1; 0 0 1])
model_index = [1 9];

model_types = {'proximal','regional','uniform'};
inset_position = [0.17 0.45 0.73];

for kk = 1:3
subplot(1,3,kk), hold on
for ii = 1:length(model_index)
axis([0 100 0 100]), ylabel('\color[rgb]{0 0 1} CcO activity (%)')
[hCcO{ii}] = plot(data.fig_3.(model_types{kk}).tissue.NO.radius,data.fig_3.(model_types{kk}).tissue.CcO{example_production,ii}.*100,'Color',[0 0 1 ii/length(model_index)],'LineWidth',2);
axis square
ax = gca;
ax.YColor = 'b'
%plot(proximal.tissue.NO.radius,O2_mmHg{model_index(ii)},'b-','Color',[0 0 1 ii/length(model_index)],'LineWidth',2)
%yyaxis right, axis([0 100 0 yheight]), title('NO in the smooth muscle [nM]')
%h1{model_index(ii)} = plot(proximal.tissue.NO.radius,proximal.tissue.NO.conc{example_production,model_index(ii)},'k-','Color',[0 0 0 ii/length(model_index)],'LineWidth',2);
end
legend([hCcO{1} hCcO{2}],{['5 \mum vessel (' num2str(round(data.fig_3.(model_types{kk}).SM.GC(72-example_production,1))) '% GC)'],['25 \mum vessel (' num2str(round(data.fig_3.(model_types{kk}).SM.GC(72-example_production,9))) '% GC)']},'Location','southwest')
title(model_types{kk})

ax1 = axes('Position',[inset_position(kk) .5 .2 .2]);
axis square
box off
hold(ax1,'on')
yyaxis left
...
yyaxis right
set(ax1,'YColor',[1 0 1])
for ii = 1:length(model_index)
yyaxis left,axis([0 100 0 yheight]), ylabel('NO (nM)')
h1{model_index(ii)} = plot(data.fig_3.(model_types{kk}).tissue.NO.radius,data.fig_3.(model_types{kk}).tissue.NO.conc{example_production,model_index(ii)},'k-','Color',[0 0 0 ii/length(model_index)],'LineWidth',2);
%plot(proximal.tissue.NO.radius,proximal.tissue.CcO{example_production,ii}.*100,'r-','Color',[1 0 0 ii/length(model_index)],'LineWidth',2)
yyaxis right
plot(data.fig_3.(model_types{kk}).tissue.NO.radius,O2_mmHg{model_index(ii)},'m-','Color',[1 0 1 ii/length(model_index)],'LineWidth',2)
yyaxis right, axis([0 100 0 50]), ylabel('O_{2} (mmHg)')
%h1{model_index(ii)} = plot(proximal.tissue.NO.radius,proximal.tissue.NO.conc{example_production,model_index(ii)},'k-','Color',[0 0 0 ii/length(model_index)],'LineWidth',2);
end
end
