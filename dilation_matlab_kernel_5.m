function [dilation] = dilation_matlab_kernel_5(c, t, NO_Mult, stim_duration, NO_sens, basalGC, Hz, radius)
HOME = pwd;
...
if (t-delay)<=0
    if c == 0 
        GC_activation_baseline = 50;
    else
    GC_activation_baseline = 100/((10^0.95/c)^0.8+1);
    end
    
    %GC activity -> relaxation
    %relaxation = GC_activation_baseline; %start with 1:1 ratio
    
    
    
%     K = GC_activation_baseline;
%     alpha = NO_sens;
%     relaxation = 100*(GC_activation_baseline.^alpha)./(K^alpha+GC_activation_baseline.^alpha); %Hill equation for non-linear relationship between GC activity and relaxation
      relaxation = 100./((10^0.95./GC_activation_baseline).^0.8+1)

%     %relaxation -> vasodilation (%)
    dilation = relaxation*(max_dilation - min_dilation)/100 + min_dilation;
%     K = GC_activation_baseline;
%     dilation = (max_dilation-min_dilation)*(relaxation.^alpha)./(K^alpha+relaxation.^alpha)+min_dilation; %Hill equation for non-linear relationship between GC activity and relaxation
...
%     b1 = 1.2;
    ...
    %get GC activity from smooth muscle from the past 6 seconds
    ...
    %GC activity -> relaxation
%     relaxation = GC_activation;
%     K = GC_activation_baseline;
%     alpha = NO_sens;
%     relaxation = 100*(GC_activation.^alpha)./(K^alpha+GC_activation.^alpha); %Hill equation for non-linear relationship between GC activity and vasodilation
    relaxation = 100./((10^0.95./GC_activation).^0.8+1)
    
    %relaxation = GC_activation; %start with 1:1 ratio
...
fileID = fopen([num2str(NO_Mult) 'NO_' num2str(stim_duration) 'NOd_' num2str(NO_sens) 'NOsens_' num2str(basalGC) 'bGC_' num2str(Hz) 'Hz_' num2str(round(radius*2,1)) 'VD_' '6sNOkernel_test_.csv'],'a');
