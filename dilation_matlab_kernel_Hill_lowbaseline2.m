function [dilation] = dilation_matlab_kernel_Hill_lowbaseline2(c, t, NO_Mult, stim_duration, NO_sens, basalGC, Hz, radius)
HOME = pwd;
...
% dilation = NO_sens_Hill(GC_activation) - NO_sens_Hill(GC_activation_baseline);

%dilation = round(dilation*NO_sens_Hill(GC_activation),8)


mult = NO_sens_Hill(GC_activation);

% if abs(dilation*NO_sens) <= 10^-6
%     dilation = 0
% elseif NO_sens_Hill(GC_activation) < 13
%     dilation = round(dilation*13,8)
% else
%     dilation = round(dilation*NO_sens_Hill(GC_activation),8)
% end

dilation = round(dilation*13,8)
% if abs(NO_sens_Hill(GC_activation)-NO_sens_Hill(GC_activation_baseline)) <= 10^-8
%     dilation = 0
% else
%     dilation = round(NO_sens_Hill(GC_activation)-NO_sens_Hill(GC_activation_baseline),8)
% end
